...
Order Processing Service
 ...
 ┌───────────────┬───────────────┐
 │               │               │
processed-orders notifications analytics
...
async function emitEvent(aggregateId, eventType, data) {
  ...
  // Show exactly what we're sending to Kafka
  console.log('\n📤 ─── PRODUCING EVENT TO KAFKA ───');
  console.log(`   Topic:       "${EVENT_TOPIC}"`);
  console.log(`   Key:         "${aggregateId}"`);
  console.log('   Value (JSON payload sent to Kafka):');
  console.log('   ┌──────────────────────────────────────────');
  ...
  console.log('   ✅ Event successfully sent to Kafka!\n');
  return event;
}
...
const eventHandlers = {
  ...
  MONEY_WITHDRAWN: (state, event) => {
    if (state.balance < event.data.amount) {
      console.warn(`   ⚠️ REJECTED! Balance $${state.balance} < Withdrawal $${event.data.amount}`);
      return state; // State unchanged — withdrawal denied
    }
    return {
      ...state,
      balance: state.balance - event.data.amount,
  ...
  await consumer.run({
    eachMessage: async ({ message }) => {
      ...
      if (handler) {
        ...
        console.log(`   State BEFORE: balance=$${currentState.balance ?? 'N/A'}`);

        const newState = handler(currentState, event);
        accountState.set(event.aggregateId, newState);

        console.log(`   State AFTER:  balance=$${newState.balance}`);
        console.log(`   Transactions: ${newState.transactions.length} total`);
        console.log('   ───────────────────────────────────\n');
      }
    },
  });
...
async function simulateBanking() {
  ...
  // ─────────────────────────────────────────────────────────────
  // EVENT 1: Create a new bank account for "John Doe"
  // ─────────────────────────────────────────────────────────────
  // This event will be sent to Kafka topic "account-events"
  // The consumer will pick it up and create initial state:
  //   { id: "ACC-xxxxx", name: "John Doe", balance: 0, transactions: [] }
  console.log('\n══════════════════════════════════════════════════');
  console.log('EVENT 1: Creating account for John Doe');
  console.log('  → Produces to topic: "account-events"');
  console.log('  → Consumer will set balance to $0');
  console.log('══════════════════════════════════════════════════');
  await emitEvent(accountId, 'ACCOUNT_CREATED', { name: 'John Doe' });
  await new Promise((r) => setTimeout(r, 1000));

  // ─────────────────────────────────────────────────────────────
  // EVENT 2: Deposit $1000 into the account
  // ─────────────────────────────────────────────────────────────
  // Consumer receives this → adds $1000 to balance
  // Expected state after: balance = $0 + $1000 = $1000
  console.log('\n══════════════════════════════════════════════════');
  console.log('EVENT 2: Depositing $1000');
  console.log('  → Previous balance: $0');
  console.log('  → Expected after:   $0 + $1000 = $1000');
  console.log('══════════════════════════════════════════════════');
  await emitEvent(accountId, 'MONEY_DEPOSITED', { amount: 1000 });
  await new Promise((r) => setTimeout(r, 1000));

  // ─────────────────────────────────────────────────────────────
  // EVENT 3: Deposit another $500
  // ─────────────────────────────────────────────────────────────
  // Consumer receives this → adds $500 to balance
  // Expected state after: balance = $1000 + $500 = $1500
  console.log('\n══════════════════════════════════════════════════');
  console.log('EVENT 3: Depositing $500');
  console.log('  → Previous balance: $1000');
  console.log('  → Expected after:   $1000 + $500 = $1500');
  console.log('══════════════════════════════════════════════════');
  await emitEvent(accountId, 'MONEY_DEPOSITED', { amount: 500 });
  await new Promise((r) => setTimeout(r, 1000));

  // ─────────────────────────────────────────────────────────────
  // EVENT 4: Withdraw $200
  // ─────────────────────────────────────────────────────────────
  // Consumer receives this → subtracts $200 from balance
  // Expected state after: balance = $1500 - $200 = $1300
  console.log('\n══════════════════════════════════════════════════');
  console.log('EVENT 4: Withdrawing $200');
  console.log('  → Previous balance: $1500');
  console.log('  → Expected after:   $1500 - $200 = $1300');
  console.log('══════════════════════════════════════════════════');
  await emitEvent(accountId, 'MONEY_WITHDRAWN', { amount: 200 });
  await new Promise((r) => setTimeout(r, 1000));

  // ─────────────────────────────────────────────────────────────
  // EVENT 5: Try to withdraw $2000 (WILL FAIL — insufficient funds!)
  // ─────────────────────────────────────────────────────────────
  // Consumer receives this → checks balance ($1300) < $2000
  // Balance stays at $1300 — withdrawal rejected
  console.log('\n══════════════════════════════════════════════════');
  console.log('EVENT 5: Attempting to withdraw $2000 (should FAIL!)');
  console.log('  → Current balance:  $1300');
  console.log('  → Requested:        $2000');
  console.log('  → $1300 < $2000 → ❌ INSUFFICIENT FUNDS');
  console.log('  → Balance stays:    $1300 (unchanged)');
  console.log('══════════════════════════════════════════════════');
  await emitEvent(accountId, 'MONEY_WITHDRAWN', { amount: 2000 });
  ...
  console.log('╠══════════════════════════════════════════════════╣');
  const finalState = accountState.get(accountId);
  console.log(`║  Account:      ${finalState.id}`);
  ...
  finalState.transactions.forEach((txn, i) => {
    const icon = txn.type === 'DEPOSIT' ? '💚 +' : '🔴 -';
    console.log(`║    ${i + 1}. ${icon}$${txn.amount} (${txn.type})`);
  });
  console.log('╠══════════════════════════════════════════════════╣');
  console.log('║  EVENT REPLAY PROOF:                             ║');
  console.log('║  $0 + $1000 + $500 - $200 = $1300 ✅            ║');
  console.log('║  (The $2000 withdrawal was rejected)             ║');
  console.log('╚══════════════════════════════════════════════════╝');
}
