...
import { ebClient } from "./eventBridgeClient.mjs";
...
export const handler  = async function(event) {
    ...
    const pathParam = pathSegments[2] || null; // "swn", "checkout", or null
    ...
    try {
      switch (event.httpMethod) {
        case "GET":
          if (pathParam) {
            body = await getBasket(pathParam); // GET /basket/{userName}
            } else {
        ...
        case "POST":
          ...
          break;
        case "DELETE":
          body = await deleteBasket(pathParam); // DELETE /basket/{userName}
          break;
        default:
