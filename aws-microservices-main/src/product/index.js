...
exports.handler = async function(event) {
    ...
    let body;
       // Parse path segments: e.g., "/product/1" → ["", "product", "1"]
    const pathSegments = event.path.split("/");
    const productId = pathSegments[2] || null; // will be "1" or null

    try {
      switch (event.httpMethod) {
        case "GET":
          if(event.queryStringParameters != null) {
            body = await getProductsByCategory(event, productId); // GET product/1234?category=Phone
          }
          else if (event.pathParameters != null) {
            body = await getProduct(productId); // GET product/{id}
          } else {
        ...
        case "POST":
          ...
          break;
        case "DELETE":
          body = await deleteProduct(productId); // DELETE /product/{id}
          break;
        case "PUT":
            body = await updateProduct(event, productId); // PUT /product/{id}
          break;
        default:
...
const getProductsByCategory = async (event, productId) => {
